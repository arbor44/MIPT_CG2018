...
def task2(im, method, threshold):
    gray = im.copy()
    gray = method(gray, threshold)
...
def forward_raw_process(im, threshold, raw, errors):
    raws = im[raw : raw + errors.shape[0]]
    
    for i in range(1, raws.shape[1]-1):
        error = raws[0, i] - float(raws[0, i] > threshold) * 255
        raws[0, i] = float(raws[0, i] > threshold) * 255
...
def backward_raw_process(im, threshold, raw, errors):
    raws = im[raw : raw + errors.shape[0]]
...
def error1_method(im, threshold=128):
    img = im.copy()
    img = add_zeros(img)
    
    for raw in range(im.shape[0]):
        img = forward_raw_process(img, threshold, raw, np.array([[0, 1]]))
    
    return img


def error2_method(im, threshold=128):
    img = im.copy()
    img = add_zeros(img)
    ...
    for raw in range(im.shape[0]):
        if raw%2 == 0:
            img = forward_raw_process(img, threshold, raw, np.array([[0, 1]]))
        if raw%2 == 1:
            img = backward_raw_process(img, threshold, raw, np.array([[1, 0]]))
    
    return img


def FS1_method(im, threshold=128):
    img = im.copy()
    img = add_zeros(img)
    
    for raw in range(im.shape[0]):
        img = forward_raw_process(img, threshold, raw, np.array([[0, 0, 7/16], [3/16, 5/16, 1/16]]))
    
    return img


def FS2_method(im, threshold=128):
    img = im.copy()
    img = add_zeros(img)
    ...
    for raw in range(im.shape[0]):
        if raw%2 == 0:
            img = forward_raw_process(img, threshold, raw, np.array([[0, 0, 7/16], [3/16, 5/16, 1/16]]))
        if raw%2 == 1:
            img = backward_raw_process(img, threshold, raw, np.array([[7/16, 0, 0], [1/16, 5/16, 3/16]]))
